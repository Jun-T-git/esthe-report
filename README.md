...
`src/score.py` で定義。「予約困難度」を 0〜100 で表現する独自スコア。

```
score = (0.7 × 完売率/100  +  0.3 × min(log(1+完売数)/log(1+CAP), 1.0)) × 100
        ────────────────────  ──────────────────────────────────────
            アクセス困難 (主軸)         需要強度 (完売数の対数加点)
```

| 成分 | 重み | 意図 |
|------|------|------|
| 完売率 | 70% | どれだけ即時アクセス困難か（高いほど予約取れない） |
| 完売数(log) | 30% | どれだけ多くの客が指名しているか（需要の絶対量） |

- **CAP=196**: 1週間の完売数の事実上の上限。これを「満点」基準として正規化。完売数がこれを超えた場合はクランプ。
- **対数で減衰**: 線形だと完売数の絶対値で順位が決まりすぎるため、対数で効きを丸めて完売率の主軸性を保つ。
- 出勤頻度・週次トレンドは本体スコアには含めず、記事側で補助情報として表示する（出勤頻度は完売数に内包され重複、トレンドは天井効果で減点される問題があるため）。

重みや CAP を変えたい場合は `score.py` の `calc_score()` を修正後、`aggregate.py --mode=all` で再集計。
...
タイトル形式：`【ランキング】データでわかるアロマモア セラピスト人気度分析 YYYY/MM/DD最新`（YYYY/MM/DD は最新集計日）
